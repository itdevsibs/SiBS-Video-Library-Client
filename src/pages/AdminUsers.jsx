...
export default function AdminUsers() {
  ...
  async function resetDevice(userId) {
    ...
    try {
      await apiFetch(`/admin/users/${userId}/reset-device`, {
        method: "POST",
      });
        ...
        >
          ...
          <div className="flex items-end">
            <button
              type="submit"
              disabled={loading}
              className="w-full rounded-2xl bg-[#FF5C28] px-4 py-3 text-sm font-extrabold text-white hover:bg-orange-600 disabled:cursor-not-allowed disabled:opacity-60"
            >
        ...
        <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="text-sm font-semibold text-slate-500">
              Manage account status and device locks.
            </p>
          </div>
                ...
                <tr key={user.id} className="border-b border-slate-100">
                  ...
                  <td className="px-4 py-4">
                    <select
                      ...
                      className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-bold outline-none"
                    >
                  ...
                  <td className="px-4 py-4">
                    <p className="max-w-[260px] truncate font-semibold text-slate-600">
                      {user.deviceId ? "Registered" : "No device"}
                    </p>
                    <p className="text-xs font-semibold text-slate-400">
                      {user.deviceLastSeen
                        ? new Date(user.deviceLastSeen).toLocaleString()
                        : "—"}
                    </p>
                  </td>
                  ...
                  <td className="px-4 py-4 text-right">
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-xs font-extrabold text-[#0D4676] hover:border-[#FF5C28] hover:text-[#FF5C28]"
                    >
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold outline-none focus:border-[#0D4676]"
      />
    </label>
  );
}
...
function Capsule({ children }) {
  return (
    <span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-extrabold uppercase text-[#0D4676]">
      {children}
    </span>
  );
}